...
4. [Установка ОС на хост (CB2 или Raspberry Pi 4B)](#глава-4--установка-ос-на-хост-cb2-или-raspberry-pi-4b)
...
- **Образ ОС для вашего хоста** — зависит от сборки (см. таблицу ниже).
- **Терминал SSH**: на Windows — встроенный `ssh` в PowerShell или [PuTTY](https://www.putty.org/); на Mac/Linux — терминал.

> 🖥 **Какая ОС на хост-компьютере (зависит от платы/хоста сборки):**
>
> | Сборка / папка | Хост | Образ ОС |
> |----------------|------|----------|
> | `klipper_config/`, `stealthchanger_config/` | BTT **CB2** в Manta M8P | **BTT CB2 V3.0.2** (Debian 12, kernel 6.1) — [bigtreetech/CB2 V3.0.2](https://github.com/bigtreetech/CB2/releases/tag/V3.0.2) |
> | `klipper_config_fly_d8/`, `klipper_config_mks_monster8/` | **Raspberry Pi 4B** | **MainsailOS 3.0** — [docs.mainsail.xyz](https://docs.mainsail.xyz/setup/mainsail-os) |
>
> Оба образа уже содержат **Klipper + Moonraker + Mainsail** (на CB2 V3.0.2 —
> в комплекте, на MainsailOS — тоже). Поэтому отдельно через KIAUH ставить их
> обычно **не нужно** — KIAUH пригодится в основном для **Katapult** и (по желанию)
> KlipperScreen / Crowsnest. Подробности — в [Главе 5](#глава-5--установка-софта).
...
- На нём установлен **Debian 12** (образ BTT CB2 V3.0.2, kernel 6.1) + Klipper + веб-интерфейс Mainsail.
- К нему вы подключаетесь по сети через браузер и по SSH.
- В сборках `klipper_config_fly_d8/` / `klipper_config_mks_monster8/` роль хоста выполняет **Raspberry Pi 4B** с **MainsailOS 3.0** (вместо CB2).
...
## Глава 4 — Установка ОС на хост (CB2 или Raspberry Pi 4B)

> Цель главы: записать Linux на карту памяти, вставить в хост, подключиться по сети.
> Образ зависит от сборки (см. таблицу ОС в [Главе 1.3](#13-программы-на-пк-скачать-заранее)).

### 4.1 Скачать образ

**Базовая сборка / StealthChanger (хост — CB2 в Manta M8P):**

1. Откройте релиз [bigtreetech/CB2 V3.0.2](https://github.com/bigtreetech/CB2/releases/tag/V3.0.2).
2. Скачайте образ **`CB2_Debian12_Klipper_kernel6.1_...img.xz`** (Debian 12, kernel 6.1,
   с предустановленным Klipper и драйверами CB2).
   > 💡 Если не нужен сервис Obico — берите вариант `..._without_obico.img.xz`.

**Сборки FLY-D8 / MKS Monster8 (хост — Raspberry Pi 4B):**

1. Скачайте **MainsailOS 3.0** — [docs.mainsail.xyz/setup/mainsail-os](https://docs.mainsail.xyz/setup/mainsail-os)
   (или выберите его прямо в Raspberry Pi Imager: *Other specific-purpose OS → 3D printing → MainsailOS*).
   В образе уже есть Klipper + Moonraker + Mainsail.
...
> ⚠ **Важно:** в рекомендованных образах (BTT CB2 V3.0.2 и MainsailOS 3.0)
> **Klipper + Moonraker + Mainsail уже предустановлены** — заново ставить их
> через KIAUH не нужно. KIAUH в этой сборке нужен в основном для **Katapult**
> (п. 5.3) и, при желании, **Crowsnest** (камера) / **KlipperScreen** (сенсорный экран).
> Если же ваш образ «голый» (без софта) — поставьте всё через KIAUH, как ниже.
...
2. Записать образ ОС на карту (CB2 V3.0.2 или MainsailOS 3.0) → вставить в хост → `ssh` ([Глава 4](#глава-4--установка-ос-на-хост-cb2-или-raspberry-pi-4b)).
3. Klipper/Moonraker/Mainsail уже в образе; через KIAUH добавить Katapult ([Глава 5](#глава-5--установка-софта)).
...
### A.9 Как пересобрать PDF

Готовый **`ИНСТРУКЦИЯ.pdf`** лежит рядом с этим файлом (картинки уже вшиты —
удобно открывать на iPad). Чтобы пересобрать его после правок (нужен Node.js
и установленный Chrome или Edge):

```bash
npm install markdown-it   # один раз
node build-pdf.js
```

Скрипт превратит `ИНСТРУКЦИЯ.md` в самодостаточный HTML (картинки → base64) и
напечатает его в `ИНСТРУКЦИЯ.pdf` через headless Chrome/Edge.

...
### Тонкая подстройка первого слоя (без перекалибровки touch)

Если слой печатается, но линии не прижаты / «висят в воздухе» — сопло чуть высоко.

В консоли (шаг **0.01–0.02 мм**):

```
SET_FIRST_LAYER_Z Z=-0.05
```

Отрицательное значение опускает сопло ближе к столу. Текущее значение хранится в `_USER_VARIABLES.gcode_z_offset` (по умолчанию **-0.03**).  
`PRINT_START` применяет его после сетки: `SET_GCODE_OFFSET Z=…`

Если сопло царапает стол — уменьшите модуль: `SET_FIRST_LAYER_Z Z=-0.02` или `Z=0`.

Постоянная калибровка Cartographer: `CARTOGRAPHER_TOUCH_CALIBRATE` → более отрицательный `z_offset` в SAVE_CONFIG.

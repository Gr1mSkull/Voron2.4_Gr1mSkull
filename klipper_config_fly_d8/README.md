# Voron 2.4 — Klipper Configuration (вариант FLY-D8 + Raspberry Pi 4B)

**Сборка:** 500×500×480 | FLY-D8 (STM32F407, CAN bridge) + Raspberry Pi 4B (MainsailOS 3.0) | EBB SB2209 CAN | Clockwork 2 / Stealthburner | Bambu X1C hotend 0.4 | Voron Tap | Cartographer (опционально)

> Отличие от базовой версии: материнская плата **FLY-D8** и хост **RPi 4B**
> (вместо Manta M8P + CB2). StealthChanger **не** используется. Тулхед,
> Tap, Cartographer, макросы — идентичны базовой версии.
...
Отредактируйте `mcu.cfg` — подставьте CAN UUID платы FLY-D8 (bridge) и EBB.
...
Добавляет `stepper_x1` (DRIVER6) и `stepper_y1` (DRIVER7) на свободные слоты FLY-D8.
